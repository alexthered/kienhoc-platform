...
@ddt.ddt
@unittest.skipUnless(settings.ROOT_URLCONF == 'lms.urls', 'Test only valid in lms')
class CreditProviderViewSetTests(AuthMixin, UserMixin, TestCase):
...
@unittest.skipUnless(settings.ROOT_URLCONF == 'lms.urls', 'Test only valid in lms')
class CreditEligibilityViewTests(AuthMixin, UserMixin, TestCase):
    list_path = 'credit:crediteligibility-list'

    def test_readonly(self):
        """ Verify the endpoint does not allow eligibility information to be created or modified. """
        self.fail()
